  ...
  if (currentTime > rcTime ) { // 50Hz
    ...
    if(rcDelayCommand == 20) {
      if(f.ARMED) {                   // actions during armed
          ...
          if (conf.activate[BOXARM] == 0 && rcSticks == (THR_CE) + YAW_LO + PIT_CE + ROL_CE) go_disarm();    // Disarm via YAW
        #endif
        #ifdef ALLOW_ARM_DISARM_VIA_TX_ROLL
          if (conf.activate[BOXARM] == 0 && rcSticks == 0X41) go_disarm();    // Disarm via ROLL
        #endif
